...
// 以下、メイン処理.リジッドボディと絡めるので、FixedUpdate内で処理を行う.
	void FixedUpdate ()
	{
		...
		// "OpenBox"フラグがtrueの時の処理
		if (anim.GetBool ("OpenBox")) {
			// 現在のステートをチェックし、ステート名が違っていたらブーリアンをfalseに戻す
			currentState = anim.GetCurrentAnimatorStateInfo (0);
			if (previousState.nameHash != currentState.nameHash) {
				anim.SetBool ("OpenBox", false);
				previousState = currentState;
			}
		}

//		if (Input.GetMouseButtonDown(0)) {
//			treasureBox.SendMessage("OpenBoxAmitaion");
//			Debug.Log("mouse x clicked!");
//
...
//		GUI.Label(new Rect(Screen.width -245,130,250,30),"Alt : LookAt Camera");
	...
	void BoxClicked() {
//		treasureBox.SendMessage("OpenBoxAmitaion");
		Debug.Log("BoxClicked called!");		
		anim.SetBool ("OpenBox", true);
	}

	void genrateTreasureBox(int num) {
		for (int i = 0; i < num; i++) {
			Vector3 position = new Vector3(Random.Range(-20, 10), 0, Random.Range(-20, 10));
